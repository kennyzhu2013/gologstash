...
func (mt MessageTemplate) PopulateMessage(msg *message.Message, subs map[string]string) error {
	...
	for field, rawVal := range mt {
		...
		default:
			//splite fields....
			//Old: only support name and representation
			//add fieldType, int,double,bool and bytes.
			//fi := strings.SplitN(field, "|", 2)
			fi := strings.SplitN(field, "|", 3)
			/*
				if len(fi) < 2 {
					fi = append(fi, "") //support null for default..
				}
			...
			*/
			...
			if len(fi) < 3 {
				...
				//support field type, must add representation..
				f := message.NewFieldEx(fi[0], val, fi[1], fi[2])
				msg.AddField(f)
			}
		}
	}
