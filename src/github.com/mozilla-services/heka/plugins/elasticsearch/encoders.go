...
func writeField(first bool, b *bytes.Buffer, f *message.Field, raw bool, replaceDotsWith string) {
	...
	case message.Field_STRING:
		...
		if len(values) > 1 {
			...
			//目前直接写json串非数组方式...即rawstring只有一个....
			if raw {
				//去掉{}操作...
				if len(values[0]) > 2 && strings.HasPrefix(values[0], "{") {
					b.WriteString(values[0][1:(len(values[0]) - 1)])
				} else {
	...
	case message.Field_BYTES:
		...
		if len(values) > 1 {
			...
			if raw {
				//去掉{}操作...
				tempString := string(values[0])
				if len(tempString) > 2 && strings.HasPrefix(tempString, "{") {
					b.WriteString(tempString[1:(len(tempString) - 1)])
				} else {
...
func (e *ESJsonEncoder) Encode(pack *PipelinePack) (output []byte, err error) {
	...
	//所有内容在{}中...
	/*
		{"Uuid":"ef8ee836-ead6-4b5c-abb4-79ec08535fe3","Timestamp":"2017-03-29T01:58:58","Type":"ReportTest",
		"Logger":"/opt/gopath/gologstash/build/heka/bin/report.log","Severity":7,"Payload":"2017-02-06 11:16:55.858 [http-nio-8080-exec-4] INFO  report \u0022activityId\u0022:9900002,\u0022event\u0022:\u0022chance\u0022,\u0022phone\u0022:\u002213802885312\u0022,\u0022province\u0022:\u0022�㶫ʡ\u0022,\u0022date\u0022:\u00222016-11-08\u0022,\u0022time\u0022:1478576140\u000d\u000a",
		"EnvVersion":"","Pid":0,"Hostname":"test","Level":"INFO","LogJson":"\u0022activityId\u0022:9900002,\u0022event\u0022:\u0022chance\u0022,\u0022phone\u0022:\u002213802885312\u0022,\u0022province\u0022:\u0022�㶫ʡ\u0022,\u0022date\u0022:\u00222016-11-08\u0022,\u0022time\u0022:1478576140","Date":"2017-02-06","Time":"11:16:55.858","Process":"[http-nio-8080-exec-4]"}
	*/
