...
// Field constructor2, valueType destination.
func NewFieldEx(name string, value string, valueString string, representation string) (f *Field) {
	var valueType Field_ValueType
	var valueResult interface{}
	var errorCode error
	switch valueString {
	case "int":
		valueType = Field_INTEGER
		valueResult, errorCode = strconv.Atoi(value)
		if errorCode != nil {
			fmt.Println("NewFieldEx:字符串转换成整数失败..")
		}
	case "double":
		valueType = Field_DOUBLE
		valueResult, errorCode = strconv.ParseFloat(value, 64)
		if errorCode != nil {
			fmt.Println("NewFieldEx:字符串转换成小数失败..")
		}
	case "bool":
		valueType = Field_BOOL
		valueResult, errorCode = strconv.ParseBool(value)
		if errorCode != nil {
			fmt.Println("NewFieldEx:字符串转换成小数失败..")
		}
	//case "bytes":
	//valueType = Field_BYTES
	default:
		valueType = Field_STRING
		valueResult = value
	}

	f = NewFieldInit(name, valueType, representation)
	f.AddValue(valueResult)
	return
}
